...
class segformer_mit_b3(nn.Module):    
    def __init__(self, in_channels, num_classes):
        ...
        self.depth_head = depth_head(in_channels=(64, 128, 320, 512), 
                                    num_classes=1, embed_dim=256)
    ...
    def forward(self, x):
        image_hw = x.shape[2:]
        x = self.backbone(x)
        seg_out = self.decoder_head(x)
        depth_out = self.depth_head(x)
        seg_out = F.interpolate(seg_out, size=image_hw, mode='bilinear', align_corners=False)
        depth_out = F.interpolate(depth_out, size=image_hw, mode='bilinear', align_corners=False)
        return seg_out, depth_out



if __name__ == "__main__":
    model = segformer_mit_b3(1,2)
