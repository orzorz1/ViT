...
class kiunet3d_1(nn.Module):  #c:输入通道数，n:基础特征通道数量

    def __init__(self, c=4, n=1, channels=128, groups=16, norm='bn', num_classes=2):
        super(kiunet3d_1, self).__init__()
...
class kiunet3d(nn.Module):  #c:输入通道数，n:基础特征通道数量
    ...
    def forward(self, x):
        ...
        out = F.interpolate(self.decoder1(out), scale_factor=2, mode='trilinear')  # U-NET
        out1 = F.max_pool3d(self.kdecoder1(out1), 2, 2) # Ki-NET
        tmp = out
        out = torch.add(out, F.interpolate(F.relu(self.interd1_1(out1)), scale_factor=0.0625, mode='trilinear'))
        out1 = torch.add(out1, F.interpolate(F.relu(self.interd1_2(tmp)), scale_factor=16, mode='trilinear'))
        ...
        out = F.interpolate(self.decoder2(out), scale_factor=2, mode='trilinear')
        out1 = F.max_pool3d(self.kdecoder2(out1), 2, 2)
        tmp = out
        out = torch.add(out, F.interpolate(self.interd2_1(out1), scale_factor=0.25, mode='trilinear'))
        out1 = torch.add(out1, F.interpolate(self.interd2_2(tmp), scale_factor=4, mode='trilinear'))

        out = torch.add(out, u1)
        out1 = torch.add(out1, o1)

        out = F.interpolate(self.decoder3(out), scale_factor=2, mode='trilinear')
        out1 = F.max_pool3d(self.kdecoder3(out1), 2, 2)
...
if __name__ == "__main__":
    model = kiunet3d( c=1, n=1, num_classes=2)
