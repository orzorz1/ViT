...
def get_norm(dir_train, dir_test):
    global global_mean, global_variance
    mean_vals = []
    var_vals = []
    pixel_counts = []
    for path in dir_train:
        print(path)
        img = read_dataset(path)
        mean_vals.append(img.mean())
        var_vals.append(img.var())
    ...
    global_mean = sum(m * n for m, n in zip(mean_vals, pixel_counts)) / total_pixels

    # 计算总体方差
    global_variance = (sum((n - 1) * v + n * (m - global_mean) ** 2 for m, v, n in zip(mean_vals, var_vals, pixel_counts)) / total_pixels)
    print("total_pixels: ",total_pixels)
    print("global_mean: ", global_mean)
    print("global_variance: ", global_variance)
...
class load_dataset_one(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = (img - global_mean) / sqrt(global_variance)
